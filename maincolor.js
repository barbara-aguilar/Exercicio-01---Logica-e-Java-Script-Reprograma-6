...
var colors =[ "#DC143C", "#CD5C5C", "#F08080", "#FF69B4", "#FF1493", "#C71585" ]
	...
	for (const [cell_index, cell] of row.entries()) {
		...
		cell.addEventListener("click", event => {
			...
			for (const cell_to_paint of matrix[row_index]) {
				cell_to_paint.style.backgroundColor = colors[Math.floor(Math.random()*colors.length)]
			}
			console.log ("row_index",row_index)
			// Agora pinte todos os elementos
			// da mesma coluna do elemento atual.
			for (const row_to_paint of matrix) {
				row_to_paint[cell_index].style.backgroundColor = colors[Math.floor(Math.random()*colors.length)]
			}
		})
	}
}